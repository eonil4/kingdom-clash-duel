/**
 * Process enemy screenshots: crop name + power ROIs, LLM-recognize those crops only,
 * WebP conversion, fileMap update.
 *
 * Uses config/llm.json userPrompt fields:
 *   - image.enemy.name  ({ prompt, roi })
 *   - image.enemy.power ({ prompt, roi })
 *
 ...
 *
 * Usage:
 *   node scripts/llm/convert2.js data/clan_clash/2026-09-18
 *   node scripts/llm/convert2.js data/clan_clash/2026-09-18 --force
 *   node scripts/llm/convert2.js data/clan_clash/2026-09-18 --clear-cache
 *   node scripts/llm/convert2.js data/clan_clash/2026-09-18 --no-clear-cache
 *
...
import { extractEnemyDataFromNamePowerCrops } from "./llm-enemy-extract.mjs";
import {
 ...
 */
async function callLLMApiForCropExtraction(imagePath, model, options = {}) {
  ...
  for (let attempt = 1; attempt <= LLM_MAX_ATTEMPTS; attempt++) {
    ...
    try {
      console.log(
        `\t[LLM crops] ${path.basename(imagePath)} (attempt ${attempt}/${LLM_MAX_ATTEMPTS}, timeout ${timeoutMs / 1000}s)...`,
      );
      const extracted = await extractEnemyDataFromNamePowerCrops(imagePath, {
        timeoutMs,
        model,
      ...
      console.log(
        `\t[LLM crops] power=${extracted.power} name=${JSON.stringify(extracted.name)}` +
          (extracted.nameLatin ? ` latin=${JSON.stringify(extracted.nameLatin)}` : "") +
      ...
      console.warn(`\t[LLM crops] attempt ${attempt}/${LLM_MAX_ATTEMPTS} failed: ${msg}`);
  ...
  throw lastErr instanceof Error
    ? lastErr
    : new Error(String(lastErr ?? "LLM crop extraction failed"));
}
...
async function processImageFolder(folderArg, options = {}) {
  ...
  console.log(
    `\nProcessing folder (convert2 / name+power crops): ${folderAbs} (${pngFiles.length} PNGs, ${force ? "force" : "missing only"})` +
  ...
  for (const originalFileName of pngFiles) {
    ...
    try {
      ...
      if (!force && existing.row && existing.webpPath && !existing.exists) {
        ...
        const { extracted, durationMs } = await callLLMApiForCropExtraction(imagePath, model, {
        ...
        const nameRaw = extracted.name?.trim() || "";
        const nameLatinRaw = extracted.nameLatin?.trim() || "";
        if (!nameRaw && !nameLatinRaw) {
          throw new Error("LLM did not return name or nameLatin.");
        }

        const mapEntry = {
          name: nameRaw || nameLatinRaw,
          nameLatin: nameLatinRaw || toSafeEnemyFilenameToken(nameRaw),
          power: Number(extracted.power),
        };

        // Filename: nameLatin (fallback to name when latin is empty). Never use english.
        const filenameNameRaw = nameLatinRaw || nameRaw;
        mapKey = canonicalEnemyMapKey(imagePath, {
          power: mapEntry.power,
          nameLatinRaw: filenameNameRaw,
          nameRaw: undefined,
  ...
  console.log(
    `\nDone (convert2): ${converted} new (LLM crops), ${convertedFromMap} from map only, ${skippedComplete} skipped (complete), ${failed} failed.${timingSuffix}`,
  );
...
async function main() {
  ...
  await processImageFolder(folder ?? "data/clan_clash/2026-09-18", {
    force,
    model,
    clearCache,
  });
